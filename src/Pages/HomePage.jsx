 ...
 return (
  <>
   ...
   <Container className="mb-5">
      ...
      {chunkArray(cardData, 2).map((row, rowIndex) => (
        <Row key={rowIndex} className="mb-1">
          {row.map((card) => (
            <Col key={card.id} md={6} lg={6} className="mb-1">
              <div className="custom-card">
                <img src={card.image} className="my-card-image" alt="User" />
                <div className="custom-card-body">
                  <p>"{card.description}"</p>
                  <p className="subline">{card.name}</p>
                  <p className='subline'>{card.aka}</p>
                </div>
              </div>
            </Col>
          ))}
